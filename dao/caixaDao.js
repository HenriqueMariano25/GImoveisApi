...
module.exports = {
    cadastrar: async (caixa, idUsuario) => {
        let insert = await db.query(`INSERT INTO caixa(
        ...
        let select = await db.query(`SELECT cai.id, cai.movimento, cai.id_debito_credito, cai.id_historico id_historico, cai.complemento_historico,
               ...
               his.descricao descricao_historico
                ...
                where cai.id = ${insert.id}`).then(resp => {
            return resp.rows[0]
        }).catch(e => {
            console.log(e)
        })
    ...
    visualizarTodos: () => {
        return new Promise((resolve, reject) => {
            db.query(`SELECT cai.id, cai.movimento, cai.id_debito_credito, cai.id_historico id_historico, cai.complemento_historico,
               imo.nome imovel_nome, imo.id id_imovel, cai.valor, cai.id_conta, cai.numero_documento, con.nome conta_nome,
               his.descricao descricao_historico
                FROM caixa cai
                LEFT JOIN imovel imo ON imo.id = cai.id_imovel
    ...
    editar: async (caixa, idUsuario) => {
        let update = await db.query(`UPDATE caixa SET movimento = '${caixa.movimento}', id_historico = '${caixa.id_historico}', 
            ...
            WHERE id = ${caixa.id} RETURNING id`).then(resp => {
            return resp.rows[0]
        }).catch(e => {
            console.log(e)
        })
        ...
        let select = await db.query(`SELECT cai.id, cai.movimento, cai.id_debito_credito, cai.id_historico id_historico, cai.complemento_historico,
               ...
               his.descricao descricao_historico
                ...
                LEFT JOIN conta con ON con.id = cai.id_conta
                LEFT JOIN historico his ON his.id = cai.id_historico
                where cai.id = ${update.id}`).then(resp => {
            return resp.rows[0]
        }).catch(e => {
            console.log(e)
        })
    ...
    deletar: async (idCaixa) => {
        let deletado = await db.query(`DELETE FROM caixa WHERE id = ${idCaixa} RETURNING id`).then(resp => {
            return resp.rows[0]
        }).catch(e => {
            console.log(e)
        })
